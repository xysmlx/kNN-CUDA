...
{
	vec.clear();
	for (int i = 0; i < (int)trainList.size(); i++)
		vec.push_back(make_pair(i, dis(dataTest, dataSet[trainList[i]])));
}
...
string KNN::MaxFreqLabel()
{
	...
	for (int i = 0; i < therK; i++)
		mp[dataSet[trainList[vec[i].first]].label]++;
